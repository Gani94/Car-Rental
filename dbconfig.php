...
if(mysqli_affected_rows($connection)==0){
	...
	mysqli_query($connection,"SELECT * FROM users_table WHERE user_name='$username'");
	if(mysqli_affected_rows($connection)==0){
	 ...
	 if(mysqli_affected_rows($connection)>0){
		...
		echo "User name is already registered. Try using another username";
	}


}else{
	echo "Email is already registered. Try using another email";
